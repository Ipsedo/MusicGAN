...
class Discriminator(nn.Module):
    ...
    def gradient_penalty(
        self, x_real: th.Tensor, x_gen: th.Tensor, alpha: float
    ) -> th.Tensor:
        ...
        grad_objective = 1.0
        grad_pen_factor = 10.0

        gradient = gradients[0].view(batch_size, -1)
        gradient_penalty: th.Tensor = (
            (gradient.norm(2, dim=1) - grad_objective) ** 2.0
        ).mean() * grad_pen_factor
...
class RecurrentDiscriminator(nn.Module):
    def __init__(self, cnn_state_dict: OrderedDict[str, th.Tensor]):
        ...
        self.__clf = nn.Linear(rnn_out_size, 1)

    def forward(self, data: th.Tensor) -> th.Tensor:
        out: th.Tensor = self.__start_block(data)
